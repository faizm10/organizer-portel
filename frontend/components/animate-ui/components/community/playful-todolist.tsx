...
const defaultCheckboxItems: PlayfulTodoItem[] = [
  {
    id: 1,
      ...
      {items.map((item, idx) => (
        <div key={item.id} className="space-y-6">
          <div className="flex items-center space-x-3">
            <motion.div
              animate={{ scale: checked[idx] ? 1.08 : 1 }}
              transition={{ type: "spring", stiffness: 320, damping: 18 }}
            >
              <Checkbox
                className="h-5 w-5 border-neutral-400 bg-white text-neutral-900 shadow-sm data-[state=checked]:bg-neutral-900 data-[state=checked]:border-neutral-900 dark:bg-neutral-900 dark:border-neutral-600 dark:data-[state=checked]:bg-neutral-100 dark:data-[state=checked]:border-neutral-100"
                checked={checked[idx]}
                onCheckedChange={(val) => {
